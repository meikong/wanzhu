...
(function($) {
	...
	jQuery.J.DrawCalendar = function(dateIn){
		...
		//Create Selector of year&month
		//var yearSelector = "<select><option value='1'>1</option><option value='2'>2</option></select>";
		...
		monthNavHead.append(jQuery('<div class="MonthName"></div>').append(d.getFullYear()+ "-"+ defaults.locale.months[d.getMonth()] ));
		...
		//Build up the Body
		var tBody = jQuery('<tbody id="CalendarBody" style="vertical-align:top;"></tbody>');
		...
		var rowCount = 0;
		var rowHeight = (defaults.height - defaults.labelHeight - defaults.navHeight-10) / Math.ceil((maxDays + Math.abs(curDay)) / 7)-30;
		...
		do {
			...
			for (var i=0; i<7; i++) {
				...
				//DateBox Events
				var dateLink = jQuery('<a>' + _currentDate.getDate() + '</a>').click(function(e) {
                    defaults.onDayLinkClick(new Date($(this).parent().attr("date")));
                    e.stopPropagation();
                });
				
				var dateBox = jQuery('<td height='+rowHeight+'></td>').attr(atts).append(dateLink).dblclick(function(e) {
		...
		var a = jQuery(ids.container).css({ "width" : defaults.width + "px", "min-height" : defaults.height + "px" });
			...
			jQuery.each(calendarEvents, function(){
				...
				if(sDt) {
					if ((sDt >= _beginDate) && (sDt <= _endDate)) {			
						...
						var height=cell.attr("height");
						cell.html(null);
						var label = jQuery(".DateLabel", cell);
						
						//var link = jQuery('<a href="' + ev.URL + '">' + cellText + '</a>');
						var link = jQuery('<a>' + cellText + '</a>');
						link.click(function(e) {
							var offset = $(this).parent().offset();
							xx = offset.left + $(this).parent().width()/2; yy = offset.top + $(this).parent().height()*4/5;	
							defaults.onEventLinkClick(ev);
							e.stopPropagation();
						});
						var event = jQuery('<div class="Event" id="Event_' + ev.EventID + '"></div>').append(link);
						var IDStr="div#Event_"+ev.EventID;
						
						
						if(ev.CssClass) { event.addClass(ev.CssClass);cell.addClass(ev.CssClass); }
						event.click(function(e) {   							
							//xx = e.pageX; yy = e.pageY;	
							var offset = $(this).offset();
							xx = offset.left + $(this).width()/2; yy = offset.top + $(this).height()*4/5;	
						...
						event.hover(function() {
							var offset = $(this).offset();
							xx = offset.left + $(this).width()/2; yy = offset.top + $(this).height()*4/5;	
							defaults.onEventBlockOver(ev);}, function() { defaults.onEventBlockOut(ev);})
